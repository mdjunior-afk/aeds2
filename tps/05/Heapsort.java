...
public class Heapsort {
    ...
    public static void main(String[] args) {
        ...
        String linha = numCompare + "\t" + numMov + "\t" + tempoExecucao;

        try (FileWriter writer = new FileWriter(matricula + "_heapsort.txt")) {
            writer.write(linha);
        } catch (IOException e) {
    ...
    public static int compareStrings(String s1, String s2) {
        ...
        for (int i = 0; i < minLength; i++) {
            if (s1.charAt(i) < s2.charAt(i)) return -1; // antes
            if (s1.charAt(i) > s2.charAt(i)) return 1; // depois
        }

        if (s1.length() < s2.length()) return -1; // antes
        if (s1.length() > s2.length()) return 1; // depois
        return 0; // iguais
    }

    public static void heapsort(Game[] games) {
        int size = games.length; // retorna o tamanho do arr

        // descobrindo a raiz do heap
        for (int i = size / 2 - 1; i >= 0; i--) {
            heapify(games, size, i);
        }

        // encontra o maior valor, coloca na posição 0 e depois faz um swap entre ele e o ultimo até estar tudo ordenado
    ...
    public static void heapify(Game[] games, int size, int i) {
        int largest = i;
        int left = 2 * i + 1;
        int right = 2 * i + 2;
        ...
        if (left < size) {
            numCompare++;
            if (games[left].estimatedOwners > games[largest].estimatedOwners) {
                largest = left;
            } else if (games[left].estimatedOwners == games[largest].estimatedOwners) {
                numCompare++;
                if (games[left].id > games[largest].id) {
                    largest = left;
                }
            }
        }
        ...
        if (right < size) {
            numCompare++;
            if (games[right].estimatedOwners > games[largest].estimatedOwners) {
                largest = right;
            } else if (games[right].estimatedOwners == games[largest].estimatedOwners) {
                numCompare++;
                if (games[right].id > games[largest].id) {
                    largest = right;
                }
            }
        }
        ...
        if (largest != i) {
            Game temp = games[largest];
            games[largest] = games[i];
            games[i] = temp;
            numMov += 3;

            heapify(games, size, largest);
        }
    }
