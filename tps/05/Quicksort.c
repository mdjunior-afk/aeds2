...
int compareDate(char *date_a, char *date_b) {
    int dayA, monthA, yearA;
    int dayB, monthB, yearB;

    sscanf(date_a, "%d/%d/%d", &dayA, &monthA, &yearA);
    sscanf(date_b, "%d/%d/%d", &dayB, &monthB, &yearB);
...
int firstGame(Game a, Game b) {
    int cmp = compareDate(a.releasedDate, b.releasedDate);
    numCompare++;
    if (cmp < 0) return 1;            // a vem antes
    numCompare++;
    if (cmp > 0) return 0;            // b vem antes
    numCompare++;
...
int main() {
    ...
    quickSort(games, 0, games_count - 1);
