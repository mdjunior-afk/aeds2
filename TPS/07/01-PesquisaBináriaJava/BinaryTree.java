...
public class BinaryTree
{
    public static void main(String[] args) 
    {
        ...
        while (true)
        {
            ...
            Game game = gameManager.findGame(path, gameID);

            tree.root = tree.insert(game.name, tree.root);
        }
        ...
        while (true)
        {
            ...
            tree.search(gameName, tree.root, "raiz ");
        }
    }
...
class BinaryTreeModel
{
    ...
    public Node insert(String value, Node node)
    {
        ...
        if (compareStrings(value, node.value) == -1)
        {
            node.left = insert(value, node.left);
        } 
        else if (compareStrings(value, node.value) == 1)
        {
    ...
    public boolean search(String value, Node node, String path)
    {
        if (node == null)
        {
            System.out.println(value + ": =>" + path + " NAO");
            return false;
        }

        if (compareStrings(value, node.value) == 0)
        {
            System.out.println(value + ": =>" + path + " SIM");
            return true;
        }

        if (compareStrings(value, node.value) == -1)
        {
            return search(value, node.left, path + " esq");
        }
        else if (compareStrings(value, node.value) == 1)
        {
            return search(value, node.right, path + " dir");
        }
