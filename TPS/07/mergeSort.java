...
public class mergeSort {
    ...
    public static void main(String[] args) {
        ...
        mergeSort(myGameList, 0, myGameList.length - 1);
        ...
        try (FileWriter writer = new FileWriter(matricula + "_mergesort.txt")) {
            writer.write(linha);
        } catch (IOException e) {
        ...
        for (int i = myGameList.length - 1; i >= myGameList.length - 5; i--) {
            myGameList[i].showInfo();
        }
    ...
    public static void mergeSort(Game[] arr, int start, int end) {
        numCompare++;
        if (start < end) {
            int mid = (start + end) / 2;
    ...
    public static void merge(Game[] arr, int start, int mid, int end) {
        int n1 = mid - start + 1;
        int n2 = end - mid;
